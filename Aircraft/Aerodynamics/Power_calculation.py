...
def compute_propeller_power(D, rho, v_o, r):
    """
    Compute propeller power for a given radius based on actuator disk theory.
    ...
    """
    A_disk = pi * r**2
    power = 0.5 * D * v_o * (np.sqrt(D / (0.5 * rho * A_disk * v_o**2) + 1) + 1)
    return power

# COMMENTED THIS OUT BECAUSE THE 1000 LINES OF TERMINAL CRAP PISSED ME OFF
