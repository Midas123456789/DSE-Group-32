...
alt_step = 1000
graph_df = [0,0,0,0,0,0,0,0]
altitudes = np.linspace(10, 20000, alt_step)

for altitude in altitudes:
    airship = Airship(3,2e5, 3, 84.5, altitude * 3.28084, 1000, 0.7)
    airship.iterate_to_exact()
    print(airship.power_required)
    row = [altitude, airship.volume, airship.wg, airship.CL, airship.CD, (airship.buoyancy/airship.D), (airship.CL/airship.CD), airship.power_required]

    graph_df = np.vstack((graph_df, row))

graph_df = pd.DataFrame(graph_df, columns=['altitude', 'Volume', 'WeightG', 'CL', 'CD', 'Lb/D', 'CL/CD', 'PowerRequired'])
...
axs[0, 2].plot(graph_df['PowerRequired'], graph_df['altitude'])
axs[0, 2].set_title('altitude vs PowerRequired')
